...
class Object3d(object):
    ...
    def __init__(self, label_file_line):
        ...
        try:
            ...
            self.score = 1.
        self.box3d = np.array(
                [data[11], data[12], data[13],data[9], data[8],data[10], data[14]]).astype(np.float32)
...
def draw_lidar(pc, fig=None, color=(1,1,1), scale=2., axis=True, show=False,z=None,name=None):
    import mayavi.mlab as mlab
    if fig is None: fig = mlab.figure(figure=name, bgcolor=(0, 0, 0), fgcolor=None, engine=None, size=(1000, 500))
    ...
    #               figure=fig)
    if z is not None:
        mlab.points3d(pc[:, 0], pc[:, 1], pc[:, 2], z,colormap="jet",mode="point",opacity=1.0,scale_factor=scale,
                      figure=fig)

    else:
        mlab.points3d(pc[:, 0], pc[:, 1], pc[:, 2], color=color,mode="point", opacity=1.0,scale_factor=scale,
                  figure=fig)
    if axis:
        # draw origin
